...
router.post("/login", async (req, res) => {
    ...
    try {
        ...
        if (userlogin) {
            ...
            console.log(isMatch + "__pass match");

            //token generate
            const token = await userlogin.generateAuthtoken();
            // console.log(token);

            res.cookie("ZCartweb", token, {
            ...
            if (!isMatch) {
                res.status(400).json({ error: "invalid passsword" })
            } else {
...
router.post("/addcart/:id", authenticate, async (req, res) => {
    try {
        ...
        if (UserContact) {
            ...
            res.status(401).json({ error: "cant add to cart at the moment" });
        }
    } catch (error) {
        res.status(401).json({ error: "cant add to cart at the moment" });
    }
